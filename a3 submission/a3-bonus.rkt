...
(define (parse-invocationTail)
  (if (check 'DEFINE)
      (begin
        (set! tokens (rest tokens))
        (set! temp (first tokens))
        (set! tokens (rest tokens))
        (list 'invocationTail (token 'DEFINE) temp (parse-expr) (begin (set! tokens (rest tokens)) (token 'CPAREN)))
        )
      (list 'invocationTail (parse-exprList) (begin (set! tokens (rest tokens)) (token 'CPAREN)))
      )
  )
...
(define (parse-invocation)
  (begin
    (set! tokens (rest tokens))
    (list 'invocation (token 'OPAREN) (parse-invocationTail)))
  )
...
(define (check-atom)
  (if (empty? tokens)
      #f
      (if (or (check 'NAME) (or (check 'STRING) (check-number)))
          #t
          #f
          )
      )
...
(define (parse-optExprList)
  (if (or (empty? tokens) (check 'CPAREN))
      (list 'optExprList)
